...
pub struct StreamServer<D> {
	uri: &'static str,
	inner: Arc<HashMap<Request, Box<dyn StreamHandler<D> + Send + Sync>>>
}
...
impl<D> StreamServer<D> {
	...
	where H: StreamHandler<D> + Send + Sync + 'static {
...
where D: Clone + Send + Sync + 'static {
	...
	fn call<'a>(
		...
		raw_data: &'a D
	) -> PinnedFuture<'a, Option<http::Result<Response>>> {
		PinnedFuture::new(async move {
			...
			let ws_key = match (header_upgrade, header_version, websocket_key) {
				(Some("websocket"), Some("13"), Some(k)) => k,
				_ => return Some(Err(ClientErrorKind::BadRequest.into()))
			};

			let ws_accept = http::ws::ws_accept(ws_key);

			let data = raw_data.clone();
			let handlers = self.inner.clone();

			let on_upgrade = http::ws::upgrade::on(hyper_req);
